...
def decode_state(encoded_state, state_counter):
    """Decodifica o estado conforme a tabela fornecida e usa o contador de estados."""
    return 'q' + str(state_counter)

def decode_transition(encoded_transition, state_counter):
    ...
    state_i = decode_state(enc_state_i, state_counter[0])
    state_counter[0] += 1
    symbol_x = decode_symbol(enc_symbol_x)
    next_state_j = decode_state(enc_state_j, state_counter[0])
    state_counter[0] += 1
...
def decode_multiple_transitions(encoded_transitions):
    ...
    decoded_transitions = []
    state_counter = [0]  # Usado para rastrear o número de estados
    
    for encoded_transition in encoded_transitions_list:
        if encoded_transition:  # Verifica se a parte não está vazia
            try:
                decoded_transition = decode_transition(encoded_transition, state_counter)
...
def decode_state(encoded_state):
    ...
    state_number = len(encoded_state) - 1
