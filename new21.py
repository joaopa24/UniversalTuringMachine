...
class MaquinaDeTuring:
    ...
    def mostrar_estado(self, transicao_index=None, valida=False, inicial=False):
        if inicial and not hasattr(self, 'inicial_mostrado'):
            ...
            print("Estado atual:")
            self.exibir_fitas(transicao_index)
            if transicao_index is not None:
                print("Transição atual:", ''.join(self.fita1[transicao_index:transicao_index + 10]))
                print("Transição", "válida." if valida else "inválida.")

    def exibir_fitas(self, transicao_index=None):
        fita1_com_transicao = ''.join(self.fita1)
        if transicao_index is not None:
            idx = 1  # Começa após o '000' inicial
            while idx < len(self.fita1) - 1:
                if idx == transicao_index:
                    fita1_com_transicao = fita1_com_transicao[:idx] + '[' + ''.join(self.fita1[idx:idx + 9]) + ']' + fita1_com_transicao[idx + 9:]
                    idx += 9
                idx += 10  # Pula para a próxima transição
